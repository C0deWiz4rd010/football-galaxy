            ...
            <TableCell colSpan={6}>
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                <div className="grid gap-3 rounded-lg bg-muted p-4 text-sm sm:grid-cols-6">
                  <span>Goals {player.stats.goals}</span><span>Assists {player.stats.assists}</span><span>Yellow {player.stats.yellowCards}</span><span>Red {player.stats.redCards}</span><span>Minutes {player.stats.minutes}</span>
