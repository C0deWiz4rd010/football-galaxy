...
import { MatchdaySelector } from './matchday-selector'
...
export function StandingsTable({
  ...
  return (
    <section className="dashboard-panel overflow-hidden rounded-[30px]">
      <div className="flex flex-col gap-3 border-b border-[var(--color-border-subtle)] px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 className="font-[var(--font-display)] text-[1.35rem] font-semibold tracking-[-0.03em] text-[var(--color-text-primary)] sm:text-[1.6rem]">
          {standings.leagueLabel}
        </h2>
        <MatchdaySelector
          currentMatchday={standings.season.currentMatchday}
          selectedMatchday={
            standings.season.selectedMatchday ?? standings.season.currentMatchday
          }
          language={language}
        ...
        <ol className="space-y-2.5 p-3.5">
          {standings.standings.map((row) => (
            <li
              key={row.team.id}
              className={[
                'dashboard-surface rounded-[22px] p-3.5 transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(3,8,15,0.28)]',
                row.position <= 4
            ...
            >
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="rounded-full bg-[linear-gradient(135deg,var(--league-accent),var(--league-accent-strong))] px-3 py-1 text-[12px] font-semibold text-[#071119] shadow-[0_14px_28px_var(--league-accent-glow)]">
                  {row.points} {copy.pointsShort}
                </div>
              </div>

              <dl className="mt-3 grid grid-cols-3 gap-2.5 text-sm text-[var(--color-text-secondary)]">
                <div className="rounded-[18px] bg-[rgba(255,255,255,0.05)] px-3 py-2.5">
